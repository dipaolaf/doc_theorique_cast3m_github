...
Ce type de schéma est celui utilisé par l'opérateur ``DYNE`` dans Cast3M
pour des problèmes vibratoires.
Pour les applications en dynamique rapide,
...
Ceci implique la création d'objet EVOLUTION en bouclant sur les pas de temps,
ou en appelant ``EVOL 'RECO'`` dans le cas de l'utilisation ``DYNE``.
...
Les opérateurs ``TFR`` et ``DSPR`` permettent de contruire les courbes
de transformées de Fourier et densité spectrales de puissance.


Exemples commentés réalisant une intégration temporelle
-------------------------------------------------------

- Dans :ref:`ex_vibrations_cloche`, est étudié l'impact d'une cloche avec ``DYNAMIC`` et ``DYNE``.
